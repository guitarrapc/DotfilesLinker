...
### フォルダー単位のリンク (`dotfiles_link_dirs`)

通常はフォルダーを再帰的にたどり、ファイルごとにリンクを作成します。指定したフォルダーを丸ごとリンクするには、リポジトリのルートに `dotfiles_link_dirs` を作成します。

```gitignore
# Codex の各スキルフォルダーをリンクし、SKILL.md は実ファイルのままにする
HOME/.agents/skills/*

# アプリケーションの設定フォルダー全体をリンクする
HOME/.config/nvim
```

パターンは `dotfiles_ignore` と同じ gitignore 形式で、コメント、ワイルドカード、末尾の `/`、`!` による除外が使えます。ただし、肯定パターンへの一致は**フォルダー単位でリンクする対象の選択**を意味します。パスは `HOME/` または `ROOT/` を含むリポジトリ相対パスです。対象はこれらの配下のフォルダーのみで、ファイル、リポジトリ直下のドットファイル、`HOME` / `ROOT` 自体の動作は変わりません。`ROOT` は Linux/macOS のみ処理されます。

- 選択したフォルダーごとに、対応する配置先へ1つのシンボリックリンクを作成します。内部は走査しません。空フォルダーにも対応し、後から追加したファイルもリンクを通じて即座に見えるようになります。
- 対象フォルダーとその親の判定では、標準の除外ルールと `dotfiles_ignore` が優先されます。一度フォルダーが選択されると**内部全体**がリンクされるため、子孫に対する ignore ルールやフォルダーリンクの除外ルールでは内部の一部を除けません。一部だけをリンクしたい場合は、親を選択せず、必要な子フォルダーを個別に指定してください。
- `dotfiles_link_dirs` が存在しない、または空の場合は、従来のファイル単位の動作になります。設定ファイル自体はホームにリンクされません。
- 同じフォルダーへのリンクが既にあればスキップします。既存の実フォルダー（ファイル単位のリンクを格納したものを含む）からの移行には `--force` が必要です。他の強制上書きと同様、リンク作成成功後は以前の配置先とその内容が削除されるため、配置先にしかないファイルは先に退避してください。
...
ルールは上から順に評価され、複数のルールに一致した場合は最後に一致したルールが優先されます。空行と `#` で始まる行は無視されます。
...
- バージョン管理システムのフォルダ/ファイル（`.git`、`.svn`、`.hg`）
