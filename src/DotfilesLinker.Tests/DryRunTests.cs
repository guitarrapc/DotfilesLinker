...
public class DryRunTests
{
    ...
    public void LinkDotfiles_WithDryRun_ShouldNotCreateLinks()
    {
        ...
        _loggerMock.Received(Arg.Is<int>(i => i > 0)).Success(Arg.Is<string>(s => s.StartsWith("[DRY-RUN]")));
    }
    ...
    public void LinkDotfiles_WithoutDryRun_ShouldCreateLinks()
    {
        ...
        _fileSystemMock.Received().CreateFileSymlink(Arg.Is<string>(s => s.Contains(".file1")), Arg.Any<string>());
        _fileSystemMock.Received().CreateFileSymlink(Arg.Is<string>(s => s.Contains(".file2")), Arg.Any<string>());
